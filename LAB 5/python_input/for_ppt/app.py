...
N = 25 # number of elements in ref string
all_rec = []
rec_no_list = []
for i in range(0,N):
    rec_no_list.append(random.choice(range(1,94)))

for rec in rec_no_list:
    all_rec.append(str(rec))
    to_be_changed = random.choice(["y","n"])
    if(to_be_changed == "y"):
        all_rec.append(to_be_changed)
        change_Str = ''.join(random.choices(string.ascii_lowercase, k=4))
