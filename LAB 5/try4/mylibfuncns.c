...
void printQueue(){
  ...
  for(int i=0;i<no_of_frames;i++){
    int f=fi->frame_no;
    printf("r=%d, m=%d, v=%d   ",PageTable[FrameTable[f]].reference_bit,PageTable[FrameTable[f]].modify_bit,PageTable[FrameTable[f]].valid_bit);
    fi=fi->next;
  }
  printf("\n");
}
...
int LRUEnchancedSecondChanceAlgorithm(int pno){
  if(check(0,0)){
    return SwapOut(queue->frame_no,FrameTable[queue->frame_no]);
  }
  if(check(0,1)){
    return SwapOut(queue->frame_no,FrameTable[queue->frame_no]);
  }
...
int check(int reference,int modify){
  ...
  for(int i=0;i<n;i++){
    ...
    if(modify){
      PageTable[FrameTable[f]].reference_bit=0;
    }
    queue=queue->next;
  }
  return flag;
