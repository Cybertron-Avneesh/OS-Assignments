...
int main(int argc,char * argv[]){
    ...
    int ans=input_array[0];
    for(int i=1;i<input_arguments_length;i++){
        ans=((input_array[i]*ans)/gcd(input_array[i],ans));
    }
    result_buf->result[0]=ans;
    result_buf->result[1]=gcd(input_array[0],input_array[1]);
